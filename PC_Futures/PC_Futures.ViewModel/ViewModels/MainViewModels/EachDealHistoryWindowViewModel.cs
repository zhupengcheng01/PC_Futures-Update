    ...
    public class EachDealHistoryWindowViewModel : ObservableObject
    {
        ...
        public void PreviousExecuteChanged()
        {
            if (EachDealHistoryList.Count > 0)
            {
                ...
                if(list.Count==0)
                {
                    MessageBox.Show("已是第一页了！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            ...
            {
                MessageBox.Show("当前集合为空，无法查询上一页！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        }
        ...
        public void NextExecuteChanged()
        {
            if(EachDealHistoryList.Count>0)
            {
                ...
                if (list.Count == 0)
                {
                    MessageBox.Show("已是最后一页了！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                EachDealHistoryList = list.ConvertAll(b => new EachDealHistoryDetailViewModel(b, _tick)).OrderBy(o => o.OrderByTime).ToList();
            }
            ...
            {
                MessageBox.Show("当前集合为空，无法查询下一页！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
        }
