...
namespace PC_Futures
{
    public class TradeLoginViewModel : ObservableObject
    {
        ...
        private void DoLoginTemp()
        {
            ...
            {
                MessageBox.Show("请输入账号/密码！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (AuthCode.ToUpper() == validCode.ReturnCode.ToUpper())
            {
                ...
                if (!scoketManager.StartTradeSocket(true))
                {
                    ...
                    ServerStatusInfoHelper.Instace().TradeServerStatus = string.Empty;
                    MessageBox.Show("服务器无法连接,请检查服务器,重新登录!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            ...
            else
            {
                AuthCode = string.Empty;
                CreateImageSource();
                MessageBox.Show("验证码输入有误，请检查后输入！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        ...
        public void HandleRecvLogin(ResultTradeLoginModel resultModel)
        {
            if (resultModel.errcode == 0)
            {
                if (!LoginBtnIsEnabled)
                {
                    if (resultModel.content != null)
                    {
                        ...
                        UserInfoHelper.AccountName = resultModel.content.account_name;
            ...
            {
                LoginBtnIsEnabled = true;
                MessageBox.Show(resultModel.errmsg, "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
        ...
        private void GetTxtSiet()
        {
            ...
            {
                MessageBox.Show("站点获取失败，请检查站点配置！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
