    ...
    public class LoginViewModel : ObservableObject
    {
        ...
        public void LoginExecuteChanged()
        {
            ...
            if (!string.IsNullOrEmpty(UserNameTemp))
            {
                ...
                {
                    MessageBox.Show("用户名或密码不正确!", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                    return;
                }
            }
            _scoketManager = ScoketManager.GetInstance();
            if (!_scoketManager.StartQuotesSocket())
            {
                MessageBox.Show("行情服务器无法连接,请检查服务器,重新登录!", "提示", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
        ...
        {
            //if (MessageBox.Show("是否确定退出", "提示", MessageBoxButton.OKCancel, MessageBoxImage.Question) == MessageBoxResult.OK)
            //{
            //    Environment.Exit(0);
            //}
            Environment.Exit(0);
